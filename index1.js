...
class SafeIntlTransformer {
  ...
  processStringLiteral(path) {
    ...
    this.recordModification(path.node.start, path.node.end, replacement);
  }
  ...
  processJSXAttribute(path) {
    ...
    if (Utils.isChinese(valuePath.node.value)) {
      // 对于JSX属性，必须使用表达式，因此统一采用intl.formatMessage形式
      const replacement = `{${this.createFormatMessage(valuePath.node.value)}}`;
      this.recordModification(valuePath.node.start, valuePath.node.end, replacement);
    }
  }
    ...
    return path.findParent(p => {
      if (t.isFunctionDeclaration(p) || 
        ...
        p.traverse({
          ...
          CallExpression(ce) {
            if (t.isIdentifier(ce.node.callee) && ce.node.callee.name.startsWith('use')) {
              isComponent = true;
            }
          }
        });
...
async function processFile(filePath) {
  try {
      ...
      FunctionDeclaration(path) {
        if (!path.scope.getBinding('intl')) {
          ...
          if (body.isBlockStatement()) {
            const intlDeclaration = parser
              .parse('const intl = useIntl();', {
                sourceType: 'module',
                plugins: ['jsx', 'typescript', 'decorators-legacy']
              })
              .program.body[0];
            body.node.body.unshift(intlDeclaration);
          }
        }
      },
      ImportDeclaration(path) {
        if (path.node.source.value === 'react-intl') {
          const needed = ['useIntl', 'FormattedMessage'].filter(x => 
            !path.node.specifiers.some(s => s.imported && s.imported.name === x)
          );
          if (needed.length > 0) {
    ...
    if (!hasReactIntlImport) {
      updatedAst.program.body.unshift(
        t.importDeclaration(
          [
